...
from initial_project.variables import ACTION, ACCOUNT_NAME, RESPONSE, MAX_CONNECTIONS, \
    ...
    RESPONSE_400, DESTIONATION, EXIT
...
def process_client_message(message, messages_list, client, clients, names):
    ...
    # Проверка: если это текстовое сообщение с содержанием, то добавляем его в очередь сообщений (ответ не требуется)
        ...
        TIME in message and MESSAGE_TEXT in message and \
            DESTIONATION in message and SENDER in message:
...
def process_message(message, names, listen_socks):
    if message[DESTIONATION] in names and names[message[DESTIONATION]] in listen_socks:
        send_message(names[message[DESTIONATION]], message)
        SERVER_LOGGER.info(f'Пользователю {message[DESTIONATION]} отправлено сообщение от пользователя {message[SENDER]}')
    elif message[DESTIONATION] in names and names[message[DESTIONATION]] not in listen_socks:
        raise ConnectionError
    else:
        SERVER_LOGGER.error(f'Пользователь {message[DESTIONATION]} не зарегистрирован на сервере. Отправка сообщения невозможна')
    ...
    while True:
        ...
        for i in messages:
            try:
                ...
                SERVER_LOGGER.info(f'Связь с клиентом {i[DESTIONATION]} потеряна')
                clients.remove(names[i[DESTIONATION]])
                del names[i[DESTIONATION]]
        messages.clear()
